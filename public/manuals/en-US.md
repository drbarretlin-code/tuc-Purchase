...
- **(Download 📤) Load from Cloud**: Download and apply content from the cloud knowledge base before editing.
- **(Export 💾) Export JSON**: Export completed specs as JSON for future editing.

---

## 3. Dashboard & Resource Monitoring 📊
Click **(Dashboard 📊)** in the top right to enter the real-time monitoring interface:
...
Utilize the top right **AI Assistant Panel (Regenerate ✨)** while filling in specs:
...
- **Threshold Adjustment (Threshold 🎚️)**: Increase for high-precision suggestions; decrease for a broader range of references.
- **One-Click Apply (Apply ➕)**: Click **(Apply ➕)** next to a suggestion to fill it directly into the field, saving typing time.

---

## 5. Preview & Multi-Language PDF Export 👁️
...
## 6. Cloud History & Maintenance ☁️
The **(Cloud History ☁️)** in the dashboard is the core of the system's knowledge:

- **Force Reparse (Force Reparse 🔄)**: If the parsed content is insufficient, check the file and click **(Repeat 🔄)** to force AI to re-scan deeply.
- **Resource Cleanup (Delete 🗑️)**: Use **(Delete 🗑️)** for old files to free up database space and reduce storage costs.
